...
describe('weatherHistory', () => {
    ...
    it('should respond an error.', async () => {
        const req = {
            query: {
              city: 'testCity',
            },
    ...
    it('should respond with daily data for the historical weather data', async () => {
        ...
        historyAxios.get.mockResolvedValue({ //intervene on URL request to create fixed responce
            data: {	
                "latitude": 52.40001,
                "longitude": 4.900009,
    ...
    it('should respond with an error when cityFetch returns a string', async () => {
        cityFetch.mockResolvedValue('City not found');
...
describe('weatherHistoryPerMonth', () => {
    it('weatherHistoryPerMonth should respond an error.', async () => {
        const req = {
            query: {
                city: 'testCity',
            },
        ...
        await weatherHistoryPerMonth(req, res);
    
        expect(res.status).toHaveBeenCalledWith(500);
        expect(res.json).toHaveBeenCalledWith({ error: 'An error occurred while fetching the weather data.' });
    });

    it('should respond with monthly data for the historical weather data', async () => {
        const req = {
            query: {
              city: 'Amsterdam'
            },
    ...
    it('should respond with an error when cityFetch returns a string', async () => {
        // Mock the cityFetch function to return a string (simulating an error)
        cityFetch.mockResolvedValue('City not found');
        ...
        // Call the weatherHistoryPerMonth function
        await weatherHistoryPerMonth(req, res);
    
        // Assert that the status and json functions are called with the correct error response
        expect(res.status).toHaveBeenCalledWith(500);
        expect(res.json).toHaveBeenCalledWith({ error: 'An error occurred while fetching the weather data.' });
      });
});