...
\subsubsection{Modules, User Tags, and Templates of the QaaS App} % Templates
...
% more threats, respond efficiently, and deal with fewer false positive alerts (\textit{\cite{xdrIDC}})
...
% can be seen as the safety team for client organizations. Important to note that SentinelOne company itself is not a \acrshort{soc}, as it
% is just a cybersecurity company that offers endpoint security solutions. Q-ICT, on the other hand, has a cybersecurity team in the department
% that can act as a SOC team or analyst to assure client's IT infrastructure to be under vigilant protection of skilled professionals.

% Figure below shows how SentinelOne as EDR is connected to other modern cybersecurity solutions:
...
% be implemented in the application because turning it on will cause them more money. Therefore, displaying all the Ranger ability
...
% \textbf{\textit{How can SentinelOne maintain endpoint monitoring even whgen the device is not connected to any network?}}

% On a network, before a machine is connected and talks to other devices and gateways, it is going to do a broadcast and gives up information
% about itself.
...
% that is not connected to the Internet. A lot of the times, most switches and routers do not change their default credentials and are not configured properly,
% which makes it easy for an attacker to get in and do a malicious act.

% As every single devices that is connected to a network is constantly broadcasting information out to the rest of
% other devices, in order to get and maintain an \acrshort{ip} address to keep accessing the Internet, even when it is not actively using
      ...
      \item \textbf{Threat}: containing information about the incidents that occurred on the client's endpoint that have been prevented by SentinelOne.
            ...
            \begin{itemize}
                  ...
                  \item Threat timeline: provides a chronological view of events related to a detected threat. This will be in form of event sequence of all events
                        ...
                        compromise.
                  \item Threat process events: are the individual actions or activities that occur as part of a detected threat. These events include various actions
                        such as: \begin{enumerate}
                              \item Files: are events involved in actions taken on files within the system. This includes modifications, deletions, creations, and other actions
                                    that might indicate malicious activities. It is important to monitor these changes of files to help maintain data integrity and detect any
                                    tampering.
                              \item Process: related actions taken by or against running processes. This includes process creation, execution, termination, and other interactions
                                    that may indicate malicious behaviour. It is crucial in identifying unusual or unauthorized process activities that deviate from normal behaviour.
                              \item Indicators: also known as \acrshort{ioc}, are pieces or forensic data that identify potentially malicious activity. These can include file hashes,
                                    IP addresses, domain names, URLs, and other artefacts associated with threats.
                              \item Network actions: network-related events such as IP connect, data transfers, port scanning, DNS request, and other malicious activities.
                        \end{enumerate}
            \end{itemize}
      \item \textbf{Potential Risky Applications}: detailing lists of outdated software that have proven to have unknown vulnerabilities in the cybersecurity world.
            With this data, clients can see which applications on their devices are outdated, allowing them to prioritize patching and updates and ensure
            compliance.
            \begin{itemize}
                  \item CVE data: will show the CVE details issued by NIST and MITRE so that the user can get a better understanding on how the outdated application
                        might be dangerous. The explanation is very technical, iterating over security bugs that may be related to the outdated application.
            \end{itemize}
...
%                               {
%                                     "type": "string",
%                                     "example": "user_action_needed_fda",
%                                     "enum": [
...
%                         "groupIp": "31.155.5.x",
%                         "lastSuccessfulScanDate": "2018-02-27T04:49:26.257525Z",
%                         "installerType": ".msi",
...
% SentinelOne has a number of limitations in its \acrshort{api}, which is 1.000 \acrshort{api} calls per minute. To combat this, the author
...
% duration can be changed later by other developers if needed.
...
In Trend Micro, tabs can be added and deleted. It will contain up to 10 visualization widgets, which are shown
in the following:
...
TrendMicro data can also be filtered by tags, which can be used to add additional attributes to the data.
...
Figure below tells us about the look of Heimdal dashboard
...
% First off, it is necessary to research the types of visualization charts that Flutter can generate.
...
% \subsection{Conclusion}
% In Flutter, there are three main libraries that support a wide range of chart types, which are: \texttt{fl\_chart},
% \texttt{flutter\_echarts}, and \texttt{syncfusion\_flutter\_charts}. Each chart type is suitable for a specific
% kind of data and provides a different way to visualize the data. The SentinelOne data will mainly be displayed by
% using pie chart and bar graph because the data is categorical and numerical. Comparing to the other \acrshort{edr}
...
% the dashboard. The dashboard should also store the user's preferences and settings, so that the user can see
